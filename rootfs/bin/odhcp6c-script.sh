...
setup_interface () {
    ...
    for entry in $PREFIXES; do
        ...
        mico_log "setup_interface: route add unreachable \"$addr\""
        ip -6 route add unreachable "$addr"
    done

    # while ip -6 route del default dev "$device" ; do
        # true
    # done
    # ip -6 address flush dev "$device" scope global
    # ip -6 route add default dev "$device"
    ...
    for entry in $ADDRESSES; do
        ...
        mico_log "setup_interface: address add \"$addr\""
    ...
    for entry in $RA_ROUTES; do
        ...
        for prefix in $PREFIXES; do
            ...
            [ -n "$gw" ] && ip -6 route add "$addr" via "$gw" metric "$metric" dev "$device"
        done
    done
}
